...
- **编辑器**：Markdown 语法着色、当前行高亮、列表/引用回车自动延续、Tab = 2 空格
...
├── Sources/MarkNote/Resources/ # preview 管线 + vendor JS（离线）
