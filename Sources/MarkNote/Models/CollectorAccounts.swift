...
enum CollectAccounts {
    ...
    /// 内置站点表（顺序 = 面板里的展示顺序）。
    ///
    /// **进表门槛（只收优质来源，2026-09 定）**：
    /// 1. 本身是**一手来源**（原创/正版/官方），不是素材二道贩子或内容农场；
    /// 2. 登录确实能让采集**多拿到东西**（原图 / 全文 / 高清 / 更多章节），而不是「只是能看」；
    /// 3. 内容**抓得到**：正文或媒体是静态/半静态可取（DRM 长视频站、纯 JS 渲染站不收）；
    /// 4. 域名稳定、至今可访问；水印站 / 有版权风险的商业图库不收。
    ///
    /// 被否掉的域名记在 `rejected`，`testRejectedDomainsAreNotInTable` 会拦住手滑加回来。
